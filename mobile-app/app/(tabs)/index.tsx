...
const PRODUITS_API = "http://localhost:8091";
const AVIS_API = "http://localhost:8092";
...
export default function App() {
  ...
  useEffect(() => {
    axios.get(`${PRODUITS_API}/api/categories`)
  ...
  const loadProduits = (categorieId) => {
    ...
    axios.get(`${PRODUITS_API}/api/produits?categorieId=${categorieId}`)
  ...
  const loadAvis = (produit) => {
    ...
    axios.get(`${AVIS_API}/api/avis/${produit.id}`)
  ...
  return (
      <View style={styles.container}>
        ...
        {categories.map(cat => (
            <Button key={cat.id} title={cat.nom} onPress={() => loadProduits(cat.id)} />
        ))}
...
const styles = StyleSheet.create({
  container: { padding: 30, marginTop: 30, flex: 1 },
  title: { fontSize: 26, fontWeight: "bold", marginBottom: 20 },
  section: { fontSize: 20, fontWeight: "bold", marginTop: 20, marginBottom: 10 },
  card: { padding: 12, backgroundColor: "#eee", marginBottom: 10, borderRadius: 8 },
  review: { padding: 10, backgroundColor: "#ddd", marginBottom: 8, borderRadius: 8 },
